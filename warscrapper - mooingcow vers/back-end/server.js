...
var db = require('./data/databaseConnect.js');
...
app.get('/items/all', function(req,res) {
    ...
    dbConn.connect(function (err) {
        if (err) {//database connection gt issue!
            ...
            const findAllitems = "SELECT * FROM items;";
            dbConn.query(findAllitems, (error, results) => {
                dbConn.end();
                if(error) {
                    res.send(error);
                };
